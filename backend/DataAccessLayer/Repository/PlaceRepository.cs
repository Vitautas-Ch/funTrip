...
namespace DataAccessLayer.Repository
{
    public class PlaceRepository : GenericRepository<Place>, IPlaceRepository
    {
        ...
        public IEnumerable<Place> FilterPlacesByCity(Func<Place, bool> predicate,
            bool includeCities = false,
            bool includeTypePlace = false,
            bool includeImageRegion = false)

        {
            ...
            if (includeTypePlace)
            {
                result = result.Include(x => x.TypePlace);
            }
            if (includeImageRegion)
            {
                result = result.Include(x => x.ImagePlaces);
            }
